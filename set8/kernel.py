...
def experiment(x_test, y_test, x_train, y_train, C=0.01, kernel='poly', Q=2, gamma=1.0, coef0=1.0, num1=None, num2=None, error_in=False, error_out=False, num_sv=False, error_cv = False):
	...
	if error_cv:
		# shuffle the data.
		output.append(error_cross_validate(SVC(C=C, kernel=kernel, degree=Q, gamma=gamma, coef0=coef0), x_train, y_train, n_fold=10))
...
def answer(q2=False, q3=False, q4=False, q5=False, q6=False, q7=False, q8=False, q9=False, q10=False):
	...
	def question2():
		...
		for num in range(0, 10, 2):
			question2.append((experiment(x_test, y_test, x_train, y_train, num1=num, error_in=True), num))
	...
	def question3():
		...
		for num in range(1, 10, 2):
			question3.append((experiment(x_test, y_test, x_train, y_train, num1=num, error_in=True), num))
	...
	def question5():
		...
		for c in (0.001, 0.01, 0.1, 1):
			result = experiment(x_test, y_test, x_train, y_train, C=c, num1=1, num2=5, error_in=True, error_out=True, num_sv=True)
	...
	def question6():
		...
		for c in (0.0001, 0.001, 0.01, 1):
			for q in (2, 5):
				result = experiment(x_test, y_test, x_train, y_train, C=c, Q=q, num1=1, num2=5, error_in=True, error_out=True, num_sv=True)
				string += "C: %.4f, " % c + "Q: %.1f" % q + "\t[e_in, e_out, num_sv]: " + str(result) + "\n"
		return string[:-1]

	def question7_8():
		nonlocal y_test, x_test, y_train, x_train
		dict_min_c = defaultdict(list)
		# 100 runs
		for _ in range(100):
			question7_8 = []
...
answer(q2=True, q3=True, q4=True, q5=True, q6=True, q7=True, q8=True, q9=True, q10=True)
